...
async def post_mentor(
    ...
    try:
        ...
        raise HTTPException(
            status_code=400,
            detail="IDs de tecnologías inválidos (usa solo números separados por comas)",
        )
...
@router.get("/{mentor_id}/technologies")
def get_mentor_technologies(mentor_id: int):
    results = get_technologies(mentor_id)

    if not results:
        raise HTTPException(
            status_code=404, detail="No se encontraron tecnologías para este mentor."
        )
